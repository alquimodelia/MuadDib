...
def prediction_score(test_dataset_Y, predictions, test_allocation, model_name):
    ...
    bscore_m = (
        (benchmark_alloc_missing - alloc_missing_sum) / benchmark_alloc_missing
    ) * 100
    bscore_s = (
        (benchmark_alloc_surplus - alloc_surplus_sum) / benchmark_alloc_surplus
    ) * 100
    ...
    predict_score = {
        ...
        "EPEA": [EPEA],
        "EPEA norm": [EPEA_norm],
        "EPEA Bench": [EPEA_Bench],
        "EPEA Bench norm": [EPEA_Bench_norm],
        "EPEA norm2": [EPEA_norm2],
...
def simple_prediction_score(
    test_dataset_Y, predictions, test_allocation, model_name, **kwargs
):
    erro = test_dataset_Y - predictions
    erro_abs = np.abs(erro)
    erro_abs_sum = np.nansum(erro_abs)
...
def prediction_score(
    ...
    if test_allocation:
        ...
        benchmark_final_truth = new_data[new_data["status"] == "new"][
            "EUR"
        ].values
        ...
        erro_final = (
            benchmark_final_truth[:min_final_len]
            - benchmark_final[:min_final_len]
        )

        slope_final_pred = (
            benchmark_final[:min_final_len][-1]
            - benchmark_final[:min_final_len][0]
        )
        slope_final_pred = slope_final_pred / len(
            benchmark_final[:min_final_len]
        )

        slope_final_truth = (
            benchmark_final_truth[:min_final_len][-1]
            - benchmark_final_truth[:min_final_len][0]
        )
        slope_final_truth = slope_final_truth / len(
            benchmark_final_truth[:min_final_len]
        )
    ...
    if benchmark_edge is not None:
        ...
        erro_edge = (
            benchmark_edge_truth[-len(benchmark_edge) :] - benchmark_edge
        )
        ...
        edge_truth_derivative = np.diff(
            benchmark_edge_truth[-len(benchmark_edge) :]
        )
        ...
        slope_edge_truth = (
            benchmark_edge_truth[-len(benchmark_edge) :][-1]
            - benchmark_edge_truth[-len(benchmark_edge) :][0]
        )
        slope_edge_truth = slope_edge_truth / len(
            benchmark_edge_truth[-len(benchmark_edge) :]
        )
...
def validate_model(
    ...
    **kwargs,
    ...
    X, Y, _, _ = datamanager.validation_data(
    ...
    if edge_value > 0:
        ...
        data_for_btc_validation_edge = data_for_btc_validation_edge.reshape(
            (1, *data_for_btc_validation_edge.shape)
        )
    ...
    predict_score = prediction_score_fn(
        Y, predictions, benchmark, model_name, num_common=num_common
    )
    benchmark = False
    save_scores(
        Y,
...
def result_validation(
    ...
    **kwargs,
    ...
    if validate_mode == "highest_stable":
        ...
        best_value_case = best_value_case[
            best_value_case[validation_target] == best_value
        ]

    elif validate_mode == "highest":
        if min_error_var:
            best_index = (
                exp_results[validation_target].abs().nsmallest(1).index
            )
        else:
    ...
    def metrics_to_save_fn(
        ...
        **kwargs,
        ...
        if metrics_to_save is None:
            metrics_to_save = [
                f for f in exp_results.columns if f not in ["name", "epoch"]
            ]

        exp_metrics = {}
        for name in unique_name:
            best_case, best_result = experiment.result_validation_fn(
                exp_results[exp_results["name"] == name],
                experiment.validation_target,
                **kwargs,
            )
            exp_metrics[name] = {}
